...
const INITIAL_POLICY = {
  ...
  contrib_exec_w: 70, contrib_bid_w: 30, contrib_core_min: 20, contrib_bonus_multi: 20, contrib_bonus_jikjik: 5, contrib_sales_floor: 20,
...
const shortDept = (n) => { n = String(n || '').replace(/^\(예시\)\s*/, ''); return n.length > 12 ? n.slice(0, 11) + '…' : n; };
...
const EVAL_CFG = { execW: 70, bidW: 30, coreMin: 20, bonusMulti: 20, bonusJikjik: 5, salesFloor: 20, bidPmW: 100, bidPartW: 60, bidSuppW: 30 };
          ...
          <div style={{ ...card(), padding: S[5], width: 420, maxWidth: '100%' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gap: S[3], marginTop: S[3] }}>
              <select value={form.empId} onChange={e => setForm(f => ({ ...f, empId: e.target.value }))} style={inp}>
                ...
                {(employees || []).map(e => <option key={e.id} value={e.id}>{e.name} ({shortDept(e.dept)})</option>)}
              </select>
              <input inputMode="numeric" placeholder="원금(원)" value={fmtInput(form.principal)} onChange={e => setForm(f => ({ ...f, principal: parseInput(e.target.value) }))} style={inp} />
              <div style={{ display: 'flex', gap: S[2] }}>
              ...
              <input inputMode="numeric" placeholder="월 상환약정액(원)" value={fmtInput(form.monthly)} onChange={e => setForm(f => ({ ...f, monthly: parseInput(e.target.value) }))} style={inp} />
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setRepay(null)}>
          <div style={{ ...card(), padding: S[5], width: 380, maxWidth: '100%' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gap: S[3], marginTop: S[3] }}>
              ...
              <input inputMode="numeric" placeholder="상환액(원)" value={fmtInput(repay.amount)} onChange={e => setRepay(r => ({ ...r, amount: parseInput(e.target.value) }))} style={inp} />
  ...
  return (
    <div>
      ...
      <select value={target} onChange={e => setTarget(e.target.value)} style={{ padding: '7px 10px', border: `1px solid ${T.border}`, borderRadius: 6, fontSize: 12.5, marginBottom: S[3] }}>
        ...
        {candidates.map(e => <option key={e.id} value={e.id}>{e.name} ({shortDept(e.dept)})</option>)}
      </select>
    ...
    onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ 
        ...
        <div style={{ padding: S[6] }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: S[4], marginBottom: S[5] }}>
            <Field label="기본급">
              <ModalInput value={fmtInput(form.baseSalary)} onChange={v => set('baseSalary', parseInput(v))} />
            </Field>
            <Field label="직책수당">
              <ModalInput value={fmtInput(form.allowance)} onChange={v => set('allowance', parseInput(v))} />
            </Field>
            <Field label="식대·차량유지비">
              <ModalInput value={fmtInput(form.mealCar)} onChange={v => set('mealCar', parseInput(v))} />
            </Field>
            <Field label="자격수당">
              <ModalInput value={fmtInput(form.qualif)} onChange={v => set('qualif', parseInput(v))} />
            </Field>
          </div>
        ...
        <div style={{ ...card(), padding: S[6] }}>
          ...
          {canEdit && (
            <div style={{ display: 'flex', gap: S[2], marginBottom: S[3] }}>
              ...
              <input value={fmtInput(amt)} onChange={e => setAmt(parseInput(e.target.value))} inputMode="numeric" placeholder="금액" style={{ flex: 1, padding: '7px 10px', border: `1px solid ${T.border}`, borderRadius: 6, fontSize: 12, fontFamily: FONT, textAlign: 'right', boxSizing: 'border-box' }} />
          ...
          <div style={{ ...card(), padding: S[5], width: 440, maxWidth: '100%' }} onClick={ev => ev.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gap: S[3], marginTop: S[3] }}>
              ...
              <div><div style={{ fontSize: 11, color: T.textMute, marginBottom: 2 }}>제안참여 인력 (가중 {EVAL_CFG.bidPartW}% · 쉼표로 구분)</div>
                <input value={edit.participants} onChange={ev => setEdit(f => ({ ...f, participants: ev.target.value }))} placeholder="미입력 시 기본: 최영숙, 이원규" style={{ width: '100%', padding: '7px 10px', border: `1px solid ${T.border}`, borderRadius: 6, fontSize: 12.5, boxSizing: 'border-box' }} /></div>
            ...
            <div style={{ fontSize: 11, color: T.textMute, marginTop: S[3], lineHeight: 1.6 }}>이 제안이 수주 확정되면 위 인력에게 역할 가중대로 수주 기여점수가 반영됩니다. 참여·지원을 비워두면 회사 표준 제안팀(참여: 최영숙·이원규 / 지원: 신수호·심도현)이 자동 적용됩니다. 대표(정일영·최재교)는 평가·기여도 제외. 가중치는 정책설정에서 조정.</div>
          </div>
        </div>
      )}
    </div>
...
function ManagementReportView({ user, projects, proposals, overheads, employees, empLedger, setEmpLedger, currentYear, policy, receivables, cashCfg, setCashCfg, upsertProject, deleteProject, fin }) {
  ...
  const empRows = Object.values(eMap).filter(e => {
    const emp = (employees || []).find(x => x.id === e.empId);
    return !emp || emp.evalTarget !== false;   // 평가 제외자(각자대표 등)는 기여도 표에서도 제외
  }).map(e => {
  ...
  return (
    <div className="report-wrap" style={{ maxWidth: 1120 }}>
            ...
            <details className="no-print" style={{ marginTop: S[3] }}>
              ...
              <div style={{ overflow: 'auto', marginTop: S[2] }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5, minWidth: 860 }}>
                  ...
                  <tbody>
                    {rows.map((r, i) => (
                      <tr key={i} onClick={() => setMonthDetail(r)} style={{ cursor: 'pointer', ...(r.balS < safety ? { background: 'rgba(180,35,24,0.05)' } : {}) }} title="클릭하면 이 달의 수입·지출·잔고 계산 상세를 봅니다">
                ...
                <div onClick={() => setMonthDetail(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
                  <div onClick={e => e.stopPropagation()} style={{ ...card(), padding: S[5], width: 560, maxWidth: '100%', maxHeight: '90vh', overflow: 'auto' }}>
                    ...
                    <div style={{ background: T.surfaceAlt, borderRadius: 8, padding: S[3], fontSize: 12, lineHeight: 1.7 }}>
                      <div style={{ fontWeight: 700, color: T.ink, marginBottom: 3 }}>📈 수주예정(파이프라인) 입금 상세</div>
                      {pipeInc > 0 ? (
                        <div>위 예측잔고에는 이 달 수주예정 사업의 선급·잔금 <strong style={{ color: T.success }}>{fmtMoney(pipeInc)}원</strong>이 포함돼 있습니다. <span style={{ color: T.textMute }}>(수주율로 가중된 기대금액)</span><br />이를 빼면(확정 수입만) 통장잔고는 <strong style={{ color: T.textMute }}>{fmtMoney(r.bal)}원</strong>입니다.</div>
                      ) : (
                        <div style={{ color: T.textMute }}>이 달은 수주예정 사업의 입금 예정이 없어, 예측잔고와 통장잔고가 동일합니다.</div>
                      )}
      ...
      {/* 6. 직원별 순기여 */}
      <details className="no-print" style={{ marginTop: S[5] }}>
        <summary style={{ fontSize: 14, fontWeight: 700, color: T.ink, cursor: 'pointer', padding: `${S[2]}px 0` }}><UserCheck size={15} color={T.brand} style={{ verticalAlign: 'middle', marginRight: 6 }} />6. 직원별 순기여 (사업 참여 기준) — 펼치기</summary>
      {canEditLedger && (
          ...
          <div style={{ ...card(), padding: S[5], width: 400, maxWidth: '100%' }} onClick={ev => ev.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gap: S[3], marginTop: S[3] }}>
              ...
              <div>
                ...
                <input inputMode="numeric" value={fmtInput(ledgerForm.newOrder)} onChange={ev => setLedgerForm(f => ({ ...f, newOrder: parseInput(ev.target.value) }))} style={{ width: '100%', padding: '7px 10px', border: `1px solid ${T.border}`, borderRadius: 6, fontSize: 12.5, boxSizing: 'border-box' }} />
              </div>
              <div>
                <div style={{ fontSize: 11, color: T.textMute, marginBottom: 2 }}>법인카드 지출(원)</div>
                <input inputMode="numeric" value={fmtInput(ledgerForm.card)} onChange={ev => setLedgerForm(f => ({ ...f, card: parseInput(ev.target.value) }))} style={{ width: '100%', padding: '7px 10px', border: `1px solid ${T.border}`, borderRadius: 6, fontSize: 12.5, boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', gap: S[2], justifyContent: 'flex-end' }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: T.surface, borderRadius: 10, width: '100%', maxWidth: 720, maxHeight: '92vh', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: `${S[5]}px ${S[6]}px`, overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: S[3], marginBottom: S[4] }}>
            <div><label style={labelStyle}>매출</label><input style={numStyle} inputMode="numeric" value={fmtInput(form.revenue)} onChange={e => set('revenue', parseInput(e.target.value))} placeholder="0" /></div>
            <div><label style={labelStyle}>작업자인건비</label><input style={numStyle} inputMode="numeric" value={fmtInput(form.workerLabor)} onChange={e => set('workerLabor', parseInput(e.target.value))} placeholder="0" /></div>
